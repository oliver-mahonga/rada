...
import { 
  ...
  Globe, Loader2, Quote, Target, Zap
} from "lucide-react";
...
export default function OnboardingPage() {
  ...
  const syncProfileToBackend = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/auth/onboarding/${userId}`, {
        method: "PATCH",
        ...
        {step === 1 && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-1000">
            <div className="text-center">
              ...
              <p className="text-[#9d96be] font-medium max-w-sm mx-auto">Define how you are perceived within the Rada Core.</p>
            </div>
            ...
            <button onClick={handleNextStep} disabled={loading} className="w-full py-5 bg-white text-black font-black uppercase tracking-[4px] text-xs rounded-2xl flex items-center justify-center gap-3 active:scale-95">
              {loading ? <Loader2 className="animate-spin" size={16} /> : <>Confirm Profile Details <ArrowRight size={16} /></>}
            </button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-700">
            <div className="text-center">
              ...
              <p className="text-[#9d96be] font-medium">Select high-income skills for your Rada path.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {skillOptions.map((skill) => (
                ...
                >
                  <div className={`p-4 rounded-2xl bg-black/40 ${skill.color}`}>{skill.icon}</div>
                  <p className="text-base font-black tracking-tight">{skill.label}</p>
                </button>
              ))}
            </div>
            ...
            >
              {loading ? <Loader2 className="animate-spin" size={16} /> : <>Generate Custom Curriculum <Sparkles size={16} /></>}
            </button>
          </div>
        )}
