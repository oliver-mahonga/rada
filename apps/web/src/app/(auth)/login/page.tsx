...
export default function LoginPage() {
  ...
  useEffect(() => { 
    setMounted(true); 
    // Auto-fill email if it was remembered previously
    const savedEmail = localStorage.getItem("rada_remembered_email");
    if (savedEmail) setEmail(savedEmail);
  }, []);

  // ── INTEGRATED AUTHENTICATION LOGIC WITH COOKIE PROTOCOL ──
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store User ID for global context
      localStorage.setItem("rada_user_id", data.user.id);

      // SET COOKIE FOR MIDDLEWARE (Server-side/Edge access)
      const cookieDuration = remember ? 60 * 60 * 24 * 30 : 60 * 60 * 24; // 30 days or 1 day
      document.cookie = `rada_token=${data.access_token}; path=/; max-age=${cookieDuration}; SameSite=Lax`;

      // 2. Handle 'Remember Me'
      if (remember) {
        localStorage.setItem("rada_remembered_email", email);
      } else {
        localStorage.removeItem("rada_remembered_email");
      }
