...
export default function EmailVerificationPage() {
  ...
  const checkVerificationStatus = async () => {
    // If userId state is empty, try to grab it directly from localStorage one last time
    const currentId = userId || localStorage.getItem("rada_user_id");

    if (!currentId) {
        console.error("Verification failed: No User ID found.");
        alert("Session Error: We couldn't find your enrollment ID. Please try signing up again.");
        return;
    }
    ...
    try {
      const response = await fetch(`http://localhost:3001/auth/verify/${currentId}`);
      ...
      if (data.verified) {
        ...
        alert("Verification still pending. If you updated the database manually, ensure the 'isVerified' column is set to true for the correct ID.");
      }
    } catch (error: any) {
      alert(`Connection failed: ${error.message}. Is the server running?`);
    } finally {
      setVerifying(false);
    }
  };
